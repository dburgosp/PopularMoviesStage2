...
public class MoviesAsyncTaskLoader extends AsyncTaskLoader<ArrayList<Movie>> {
    ...
    protected void onStartLoading() {
        if (moviesArray != null) {
            Log.i(TAG, "(onStartLoading) Reload existing previous movies array.");
            deliverResult(moviesArray);
        } else {
            Log.i(TAG, "(onStartLoading) Load new movies array.");
            forceLoad();
        }
    }
    ...
    public ArrayList<Movie> loadInBackground() {
        ...
        return NetworkUtils.fetchMovies(url);
    }
    ...
    public void deliverResult(ArrayList<Movie> data) {
        if (data == null)
            Log.i(TAG, "(deliverResult) No movies to deliver.");
        else
            Log.i(TAG, "(deliverResult) " + data.size() + " movie(s) delivered.");
        moviesArray = data;
